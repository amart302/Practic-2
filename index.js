...
let cont = document.getElementById('contein');
document.body.style.backgroundColor = 'rgb(234, 233, 233)'
for(let i = 0; i < cats.length; i++){
    ...
    age.innerHTML = `Возраст: ${cats[i].age}`;

    document.getElementById(i).appendChild(age);

    let info = document.createElement('p');
    info.innerHTML = cats[i].description;
    info.style.fontSize = '17px';

    document.getElementById(i).appendChild(info);

    let rate = document.createElement('p');
    rate.style.fontSize = '17px';
    rate.innerHTML = `Оценка: ${cats[i].rate}`;

    document.getElementById(i).appendChild(rate);

    let favour = document.createElement('p');
    favour.style.cursor = 'pointer';
    favour.innerHTML = '🖤'
    if(cats[i].favourite == true){
        favour.innerHTML = '❤️'
    }

    document.getElementById(i).appendChild(favour);
    console.log(div);
}